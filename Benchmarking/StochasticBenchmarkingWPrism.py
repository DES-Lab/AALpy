...
for seed in range(1, 4):
    ...
    for file in files:
        ...
        mdp_results, mdp_err = model_check_experiment(
            ...
            get_correct_prop_values(exp_name),
            learned_mdp,
        )
        ...
        property_string_mdp = ",".join(
            [f"{str(mdp_results[p])},{str(mdp_err[p])}" for p in mdp_results.keys()]
        )
        property_string_smm = ",".join(
            [f"{str(smm_results[p])},{str(smm_err[p])}" for p in smm_results.keys()]
        )

        text_file.write(
            "Exp_Name, n_c, n_resample, Final Hypothesis Size, Learning time,"
            "Eq. Query Time, Learning Rounds, #MQ Learning, # Steps Learning,"
            f"# MQ Eq.Queries, # Steps Eq.Queries , {properties_string_header}\n"
        )

        text_file.write(
            f'learned_mdp_{exp_name},{n_c},{n_resample}, {data_mdp["automaton_size"]}, '
        ...
        text_file.write(
            f'learned_smm_{exp_name},{n_c},{n_resample}, {data_smm["automaton_size"]}, '
