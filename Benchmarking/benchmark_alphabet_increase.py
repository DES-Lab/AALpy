...
for i in range(num_increases):
    ...
    for _ in range(repeat):
        ...
        dfa = generate_random_dfa(
            num_states, alphabet=alphabet, num_accepting_states=num_states // 2
        )
        sul = DfaSUL(dfa)
        ...
        mealy = generate_random_mealy_machine(
            num_states, input_alphabet=alphabet, output_alphabet=alphabet
        )
        ...
        eq_oracle = RandomWalkEqOracle(
            alphabet, sul_mealy, num_steps=10000, reset_prob=0.09
        )
        ...
        moore = generate_random_moore_machine(
            num_states, input_alphabet=alphabet, output_alphabet=alphabet
        )
        ...
        eq_oracle = RandomWalkEqOracle(
            alphabet, moore_sul, num_steps=10000, reset_prob=0.09
        )
