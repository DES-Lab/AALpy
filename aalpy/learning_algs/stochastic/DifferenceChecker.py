...
chi2_table[0.95] = dict(
    [
        ...
        (11, 19.67513757268249),
        (12, 21.02606981748307),
        (13, 22.362032494826934),
        (14, 23.684791304840576),
        (15, 24.995790139728616),
        (16, 26.29622760486423),
        (17, 27.58711163827534),
        (18, 28.869299430392623),
        (19, 30.14352720564616),
        (20, 31.410432844230918),
    ]
)
chi2_table[0.99] = dict(
    [
        ...
        (9, 21.665994333461924),
        (10, 23.209251158954356),
        (11, 24.724970311318277),
        (12, 26.216967305535853),
        (13, 27.68824961045705),
        (14, 29.141237740672796),
        (15, 30.57791416689249),
        (16, 31.999926908815176),
        (17, 33.40866360500461),
        (18, 34.805305734705065),
        (19, 36.19086912927004),
        (20, 37.56623478662507),
    ]
)

chi2_table[0.999] = dict(
    [
        (1, 10.827566170662733),
        (2, 13.815510557964274),
        (3, 16.26623619623813),
        (4, 18.46682695290317),
        (5, 20.515005652432873),
        (6, 22.457744484825323),
        (7, 24.321886347856854),
        (8, 26.12448155837614),
        (9, 27.877164871256568),
        (10, 29.58829844507442),
        (11, 31.264133620239985),
        (12, 32.90949040736021),
        (13, 34.52817897487089),
        (14, 36.12327368039813),
        (15, 37.69729821835383),
        (16, 39.252354790768464),
        (17, 40.79021670690253),
        (18, 42.31239633167996),
        (19, 43.82019596451753),
        (20, 45.31474661812586),
    ]
)
...
def compute_epsilon(alpha1, n1):
    epsilon1 = sqrt((1.0 / (2 * n1)) * log(2.0 / alpha1))
    return epsilon1
...
class ChiSquareChecker(DifferenceChecker):
    ...
    def check_difference(self, c1_out_freq: dict, c2_out_freq: dict, **kwargs) -> bool:
        ...
        if dof not in self.chi2_values.keys():
            raise ValueError(
                "Too many possible outputs, chi2 table needs to be extended."
            )
        else:
    ...
    def compute_Q(self, c1_out_freq, c2_out_freq, keys):
        ...
        yates_correction = (
            ...
            if len(keys) == 2
            and any(
                c1_out_freq.get(k, 0) < 5 or c2_out_freq.get(k, 0) < 5 for k in keys
            )
            else 0
        )
        for k in keys:
            p_hat_k = float(
                c1_out_freq.get(k, default_val) + c2_out_freq.get(k, default_val)
            ) / (n_1 + n_2)
            q_1_k = float(
                (
                    (abs(c1_out_freq.get(k, default_val) - n_1 * p_hat_k))
                    + yates_correction
                )
                ** 2
            ) / (n_1 * p_hat_k)
            q_2_k = float(
                (
                    (abs(c2_out_freq.get(k, default_val) - n_2 * p_hat_k))
                    + yates_correction
                )
                ** 2
            ) / (n_2 * p_hat_k)
