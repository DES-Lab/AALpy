...
def longest_prefix_cex_processing(
    s_union_s_dot_a: list, cex: tuple, closedness="suffix"
):
    """
    ...
    suffixes = (
        all_suffixes(trimmed_suffix)
        if closedness == "suffix"
        else all_prefixes(trimmed_suffix)
    )
    suffixes.reverse()
    return suffixes


def rs_cex_processing(
    sul: SUL, cex: tuple, hypothesis, suffix_closedness=True, closedness="suffix"
):
    ...
    while True:
        ...
        if (
            mq[-1] == cex_out[-1]
        ):  # only check if the last element is the same as the cex
            lower = mid + 1
            if upper < lower:
                suffix = tuple(d[1:])
                break
    ...
    if suffix_closedness:
        suffixes = (
            all_suffixes(suffix) if closedness == "suffix" else all_prefixes(suffix)
        )
        suffixes.reverse()
